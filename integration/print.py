...
запустить процессом и надеяться, что она поймёт флаги. PbReader — библиотека
этого же Python: она импортируется. Поэтому SUMATRA_PATH и FOXIT_PATH из
config.py больше ни на что не влияют, а нового пути взамен не появилось.
Ставится один раз:  pip install pbreader
...
try:
    ...
    _fail(
        ...
        f"Установите: pip install pbreader"
    )
